...
export async function POST(req: Request) {
  try {
    const { email, message, image, user_id } = await req.json();

    // Support both old format (user_id) and new format (email)
    const userEmail = email || user_id;
    ...
    if (image && userEmail) {
      const currentCount = await checkImageCount(userEmail);
      if (currentCount >= 3) {
        return NextResponse.json(
    ...
    if (image && userEmail) {
      ...
      await incrementImageCount(userEmail);
    } else {
